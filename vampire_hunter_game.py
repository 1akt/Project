...
class Weapon:
    def __init__(self, name, damage, attackRange, attackSpeed):
        self.name = name
        self.damage = damage
...
playerInventory = [
    Weapon("Dagger", 15, 1, 1.5),
    Weapon("Stake", 25, 1, 1.2)
]
currentWeaponIndex = 0
...
itemImages = {
    ...
    "Stake": pygame.transform.scale(pygame.image.load('Stake.png'), (60, 60))
}
...
def canMove(newCol, newRow):
    return (0 <= newCol < roomHeight and 0 <= newRow < roomWidth and room[newCol][newRow] == ' ')
...
def itemGeneration(x, y):
    ...
    if room[y][x] == ' ':
        ...
        return itemGeneration(random.randint(0, roomWidth - 1), random.randint(0, roomHeight - 1))
...
def dungeonGen(roomNumber):
    ...
    if roomNumber not in roomData:
        ...
        room = [['-' for _ in range(roomWidth)] for _ in range(roomHeight)]
...
class Boss(Character):
    ...
    def canBossMove(self, nextTile):
        return 0 <= nextTile[0] < roomWidth and 0 <= nextTile[1] < roomHeight and room[nextTile[1]][nextTile[0]] == ' '
    ...
    def findPath(self, start, goal):
        ...
        gScore = {start: 0}
        while openSet:
            current = min(openSet, key=lambda node: gScore[node] + self.heuristicCostEstimate(node, goal))
            if current == goal:
                return self.remakePath(cameFrom, current)
            openSet.remove(current)
            for nextTile in [(current[0] + dx, current[1] + dy) for dx, dy in [(-1,0),(1,0),(0,-1),(0,1)]]:
...
class BossHealthBar:
    ...
    def draw(self, screen):
        pygame.draw.rect(screen, (255, 255, 255), (screenWidth - self.width - 20, screenHeight - 40, self.width + 10, self.height + 10))
        current_width = int(self.width * (self.boss.health / self.boss.max_health))
        pygame.draw.rect(screen, self.colour, (screenWidth - self.width - 15, screenHeight - 35, current_width, self.height))
...
def enemyBehaviour(enemy):
    global player
    enemyDistance = calculateDistance(enemy.x, player.x, enemy.y, player.y)
    if enemy.attackCooldown == 0 and enemyDistance <= 1:
        player.health -= 10
...
def displayGameOver():
    ...
    blurredBackground = pygame.image.fromstring(blurredImage.tobytes(), blurredImage.size, blurredImage.mode)
    screen.fill((0, 0, 0))
...
def playerInteraction():
    ...
    for enemy in enemyPositions.get(roomNumber, []):
        ...
        if enemyDistance <= currentWeapon.attackRange:
            damage = currentWeapon.calculateDamage(player.level)
            enemy.health -= damage
            displayCommentary(f"Dealt {round(damage)} damage to enemy!")
        if enemy.health <= 0:
    ...
    for enemy in enemiesToRemove:
        if enemy in enemyPositions[roomNumber]:
            enemyPositions[roomNumber].remove(enemy)
...
def switchWeapon(index):
    ...
    if 0 <= index < len(playerInventory):
...
while True:
    ...
    if not gameOver:
        for enemy in enemyPositions.get(roomNumber, []):
            ...
            if enemyDistance < 2 and not playerDoor:
                newX, newY = enemy.x, enemy.y
                if enemy.x < currentRow: newX += 1
                elif enemy.x > currentRow: newX -= 1
                if enemy.y < currentCol: newY += 1
                elif enemy.y > currentCol: newY -= 1
